...
class StyleAugment:
    ...
    def add_style(self, loader, multiple_styles=False, name=None):
        ...
        if name is not None:
            self.styles_names.append(
                [name] * self.n_images_per_style if multiple_styles else [name]
            )
    ...
    def compute_size(self, amp_shift):
        ...
        b = (
            (np.floor(np.amin((h, w)) * self.L)).astype(int)
            if self.b is None
            else self.b
        )
    ...
    def _apply_style(self, img):
        ...
        img_np__ = np.clip(np.round(img_np_), 0.0, 255.0)
    ...
    def test(self, images_np, images_target_np=None, size=None):
        ...
        fft_np_ = np.fft.fft2(
            images_np if images_target_np is None else images_target_np, axes=(-2, -1)
        )
        ...
        img_np__ = np.clip(np.round(img_np_), 0.0, 255.0)
        Image.fromarray(
            np.uint8(images_target_np.transpose((1, 2, 0)))[:, :, ::-1]
        ).show()
