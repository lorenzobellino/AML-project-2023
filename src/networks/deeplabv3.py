...
def deeplabv3_mobilenetv2(num_classes: int = 21, in_channels: int = 3) -> DeepLabV3:
    width_mult = 1
    ...
    if in_channels == 4:
        ...
        aux[:, 0, :, :] = copy.deepcopy(
            state_dict_updated["features.0.0.weight"][:, 0, :, :]
        )
        aux[:, 1, :, :] = copy.deepcopy(
            state_dict_updated["features.0.0.weight"][:, 1, :, :]
        )
        aux[:, 2, :, :] = copy.deepcopy(
            state_dict_updated["features.0.0.weight"][:, 2, :, :]
        )
        aux[:, 3, :, :] = copy.deepcopy(
            state_dict_updated["features.0.0.weight"][:, 2, :, :]
        )
