...
class SelfTrainingLoss(nn.Module):
    ...
    def __init__(
        self, conf_th=0.9, fraction=0.66, ignore_index=255, lambda_selftrain=1, **kwargs
    ):
        super().__init__()
    ...
    def get_image_mask(self, prob, pseudo_lab):
        ...
        mask_prob = (
            max_prob > self.conf_th
            if 0.0 < self.conf_th < 1.0
            else torch.zeros(max_prob.size(), dtype=torch.bool).to(max_prob.device)
        )
        ...
        if 0.0 < self.fraction < 1.0:
            for c in pseudo_lab.unique():
                ...
                max_prob_c[~mask_c] = 0
                _, idx_c = torch.topk(
                    max_prob_c.flatten(), k=int(mask_c.sum() * self.fraction)
                )
    ...
    def get_batch_mask(self, pred, pseudo_lab):
        ...
        mask = torch.stack(
            [
                self.get_image_mask(pb, pl)
                for pb, pl in zip(F.softmax(pred, dim=1), pseudo_lab)
            ],
            dim=0,
        )
        return mask
